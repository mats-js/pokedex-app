...
let pokemonRepository = (function () {
	...
	function add(item) {
		...
		if (typeof item !== 'object') {
			...
			alert(
				'New pokemon must be entered with item keys name and detailsUrl'
			);
	...
	function addListItem(pokemon) {
		...
		let button = $(
			`<button type="button" class="btn list-group-item list-group-item-action list-group-item-light" data-toggle="modal" data-target="#exampleModal">${pokemon.name}</button>`
		);
				...
				json.results.forEach(function (item) {
					let pokemon = {
						name:
							...
							item.name.slice(1), // change name's first char to uppercase
						detailsUrl: item.url,
					};
					add(pokemon);
				});
